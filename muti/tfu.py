...
def fit_by_feature(model, features_target, features_dict, targety, sample_df_in, plot_dir=None, num_quantiles=10,
    ...
    for target in features_target.keys():
        ...
        co = sample_df.groupby(target)['yh'].mean().sort_values(ascending=False).index
        
        layout = go.Layout(title='Model and Actual values by ' + target,
                           xaxis=dict(title=target, categoryorder='array', categoryarray=co),
                           yaxis=dict(title=targety))
        
        figx = go.Figure(fig, layout=layout)
        figx.update_layout(boxmode='group')
        figx.show()
        
        co = sample_df.groupby(target)[[targety, 'yh']].mean()
        fig1 = [go.Scatter(x=co['yh'], y=co[targety], mode='markers', name='',
                           customdata=co.index, marker=dict(color='black'),
                           hovertemplate='%{customdata}<br>Model %{x}<br>Actual %{y}')]
        for indx in co.index:
            i = sample_df[target] == indx
        ...
        if plot_dir is not None:
            ...
            fname = plot_dir + target + 'ModelFitCrossMean.png'
